...
image_t* renderer_get_image()
{
    ...
    for (y = 0; y < image->height; y++) {
        ...
        for (x = 0; x < image->width; x++) {
            ...
            if (section_index < 0)
                section_index = 0;
            else if (section_index > 11)
                section_index = 11;

            // Apply floyd-steinberg dithering. Transparent pixels have no error so
            // ignore them
            if (color_buffer[fb_x][fb_y] != TRANSPARENT) {
                ...
                if (fb_x < FRAME_BUFFER_SIZE - 1)
                    luminance_buffer[fb_x + 1][fb_y] += error * 7.0 / 16.0;
                if (fb_x > 0 && y < FRAME_BUFFER_SIZE - 1)
                    luminance_buffer[fb_x - 1][fb_y + 1] += error * 3.0 / 16.0;
                if (fb_y < FRAME_BUFFER_SIZE - 1)
                    luminance_buffer[fb_x][fb_y + 1] += error * 5.0 / 16.0;
                if (fb_x < FRAME_BUFFER_SIZE - 1 && fb_y < FRAME_BUFFER_SIZE - 1)
                    luminance_buffer[fb_x + 1][fb_y + 1] += error * 1.0 / 16.0;
            }
            // Calculate final palette index for this pixel
            image->data[y][x] = palette_remap_section_index(color_buffer[fb_x][fb_y],
...
void renderer_remap_color(uint8_t source, uint8_t dest)
{
    ...
    for (x = 0; x < FRAME_BUFFER_SIZE; x++)
        for (y = 0; y < FRAME_BUFFER_SIZE; y++) {
            if (color_buffer[x][y] == source)
                color_buffer[x][y] = dest;
        }
}
...
// There's lots of linear interpolation of vectors to do, and this helps limit
// the amount of code
typedef struct {
    Vector current;
    Vector step;
...
/*Returns the indices of the first and last pixel enclosed int the range
start-end.
...
void rasterize_line(primitive_t* primitive)
{
    ...
    int steep = dy > dx;
    // If not steep, we step over x coordinate, otherwise, step over y. Either
    // way, the first vertex must have the smaller coord
...
void rasterize_primitive(primitive_t* primitive)
{
    ...
    if (primitive->vertices[0].Y < primitive->vertices[1].Y)
        bits |= 4;
    if (primitive->vertices[1].Y < primitive->vertices[2].Y)
        bits |= 2;
    if (primitive->vertices[0].Y < primitive->vertices[2].Y)
        bits |= 1;
    ...
    int longest_side_right = middle_vertex.X < lerp(top_vertex.X, bottom_vertex.X,
                                                   (middle_vertex.Y - top_vertex.Y) / (bottom_vertex.Y - top_vertex.Y));
    ...
    for (i = 0; i < 2; i++) {
        ...
        for (y = y_start; y <= y_end; y++) {
            ...
            get_enclosed_pixels(left_position.X, right_position.X, &x_start, &x_end,
                &x_skip);
            u_step = 1 / (right_position.X - left_position.X);
            linear_interp_t cur_position_interp = linear_interp_init(
                left_position, right_position, x_skip * u_step, u_step);
            linear_interp_t cur_normal_interp = linear_interp_init(
                left_normal, right_normal, x_skip * u_step, u_step);
        ...
        get_enclosed_pixels(middle_vertex.Y, bottom_vertex.Y, &y_start, &y_end,
            &y_skip);
        // Compute new interpolation for the side that has changed
        u_step = 1 / (bottom_vertex.Y - middle_vertex.Y);
        if (longest_side_right) {
            left_position_interp = linear_interp_init(middle_vertex, bottom_vertex,
                y_skip * u_step, u_step);
            left_normal_interp = linear_interp_init(middle_normal, bottom_normal,
                y_skip * u_step, u_step);
        } else {
            right_position_interp = linear_interp_init(middle_vertex, bottom_vertex,
                y_skip * u_step, u_step);
            right_normal_interp = linear_interp_init(middle_normal, bottom_normal,
                y_skip * u_step, u_step);
        }
    }
}
...
void renderer_render_model(model_t* model, Matrix model_view)
{
    ...
    transform_vectors(model_view_projection, model->vertices,
        transformed_vertices, model->num_vertices, 1.0);
    // Transform normals into world space (inverse transpose is broken but not
    // actually needed)
    // Matrix normalTransform=MatrixTranspose(MatrixInverse(modelView));
    transform_vectors(model_view, model->normals, transformed_normals,
        model->num_normals, 0.0);
...
void renderer_cut(Vector point, Vector normal)
{
    ...
    for (int x = 0; x < FRAME_BUFFER_SIZE; x++)
        for (int y = 0; y < FRAME_BUFFER_SIZE; y++) {
            Vector pixel_to_point = VectorSubtract(
                VectorFromComponents(x, y, depth_buffer[x][y]), projected_point);
            if (VectorDotProduct(pixel_to_point, projected_normal) < 0)
                color_buffer[x][y] = TRANSPARENT;
        }
}
...
face_t* renderer_get_face_by_point(model_t* model,
    ...
    float* depth)
{
    // Transform model into screen space, reducing the test for intersection to a
    // 2D problem
    Vector* transformed_vertices = malloc(model->num_vertices * sizeof(Vector));
    model_view = MatrixMultiply(model_view, model->transform);
    Matrix model_view_projection = MatrixMultiply(projection, model_view);
