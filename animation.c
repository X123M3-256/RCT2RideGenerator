...
//If valid identifier not found, error
    ...
    else instruction.opcode=index-2;
...
animation_instruction_t instruction;
    ...
    break;
    case '-':
        if(state->position==0||state->str[state->position-1]=='('||state->str[state->position-1]=='+'||state->str[state->position-1]=='-'||state->str[state->position-1]=='*'||state->str[state->position-1]=='/')instruction.opcode=OP_MINUS;
        else instruction.opcode=OP_SUB;
    break;
    case '*':
...
{
animation_instruction_t instruction;
        if(state->str[state->position]=='+'||state->str[state->position]=='-'||state->str[state->position]=='*'||state->str[state->position]=='/')instruction=parse_operator(state);
        else if((state->str[state->position]>='0'&&state->str[state->position]<='9')||state->str[state->position]=='-')instruction=parse_literal(state);
        else if(state->str[state->position]>='a'&&state->str[state->position]<='z')instruction=parse_identifier(state);
        ...
        else state->error="Invalid character";
...
void instruction_list_add(instruction_list_t* list,animation_instruction_t instruction)
{
    if(list->allocated_instructions==list->num_instructions)
    {
    list->allocated_instructions*=2;
    list->instructions=realloc(list->instructions,list->allocated_instructions*sizeof(animation_instruction_t));
    }
list->instructions[list->num_instructions]=instruction;
list->num_instructions++;
}
animation_instruction_t* instruction_list_get_instructions(instruction_list_t* list,int* num_instructions)
{
animation_instruction_t* instructions=realloc(list->instructions,list->num_instructions*sizeof(animation_instruction_t));
...
void animation_expression_parse(animation_expression_t* expr,const char* str,const char** error)
{
...
strcpy(expr->str,str);
free(expr->instructions);
expr->instructions=instruction_list_get_instructions(instruction_list,&(expr->num_instructions));
}
...
int animation_object_set_parent(animation_object_t* object,animation_object_t* parent)
{
    ...
    animation_object_t* cur_object=parent;
        while(cur_object!=object&&cur_object->parent!=NULL)cur_object=cur_object->parent;
        if(cur_object==object)return 0;
    }
...
Matrix object_global_transforms[animation->num_objects];
    ...
    animation_object_t* current_object=animation->objects[i];
        ...
        {
        object_global_transforms[i]=MatrixMultiply(current_object->parent->transform,object_global_transforms[i]);
        current_object=current_object->parent;
        }
    }
//Transfer global transforms to objects
    for(int i=0;i<animation->num_objects;i++)animation->objects[i]->transform=object_global_transforms[i];
}
...
animation_calculate_object_transforms(animation,variables);
    for(int i=0;i<animation->num_objects;i++)renderer_render_model(animation->objects[i]->model,MatrixMultiply(model_view,animation->objects[i]->transform));
}
...
data.cur_object=0;
    for(int i=0;i<ANIMATION_NUM_VARIABLES;i++)data.variables[i]=variables[i];
...
animation_calculate_object_transforms(animation,variables);
     ...
     for(int i=0;i<animation->num_objects;i++)
     {
        ...
        {
        renderer_render_model(animation->objects[i]->model,MatrixMultiply(model_view,animation->objects[i]->transform));
        }
     }
return data;
}
...
animation_object_t* second_rider=NULL;
    ...
    while(data->cur_object<animation->num_objects)
    {
        ...
        {
            if(first_rider==NULL)first_rider=animation->objects[data->cur_object];
...
//Render pair of riders
    ...
    {
    renderer_render_model(second_rider->model,MatrixMultiply(data->model_view,second_rider->transform));
    renderer_remap_color(COLOR_PEEP_REMAP_1,COLOR_PEEP_REMAP_2);
    }
    if(first_rider!=NULL)renderer_render_model(first_rider->model,MatrixMultiply(data->model_view,first_rider->transform));
}

int animation_count_riders(animation_t* animation)
{
int riders=0;
    for(int i=0;i<animation->num_objects;i++)if(animation->objects[i]->model->is_rider)riders++;
return riders;
}
